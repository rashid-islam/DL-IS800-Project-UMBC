...
train_data_ae =  convert_ae(train_ratings, numUsers, numItems)
#train_data_iae = np.transpose(train_data_uae)
val_data_ae =  convert_ae(val_ratings, numUsers, numItems)
test_data_ae =  convert_ae(test_ratings, numUsers, numItems)
...
import sys
sys.stdout=open("results/user_based_ae_results.txt","w")
  ...
  for layers in hidden_layers:
    for lr in learning_rate: 
        for d_out in drop_probs:          
            ...
            ae = deepAutoencoder(numItems, vec, layers, d_out).to(device)
...
torch.save(best_uae.state_dict(), "trained-models/best_uae")
#%% evaluate on the test set
print('\n')
print(f"Evaluation on the test dataset:")
test_model(best_uae,train_data_ae,test_ratings)
avg_HR, avg_NDCG = evaluate_ranking(best_uae,train_data_ae,test_ratings.values,top_K,random_samples, numItems,interactions)

np.savetxt('results/avg_HR_uae.txt',avg_HR)
np.savetxt('results/avg_NDCG_uae.txt',avg_NDCG)
